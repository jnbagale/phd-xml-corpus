...
do
	...
	echo "${file}"
	echo "Encode"
	/usr/bin/time -o exi_default_encode.txt -a -f "%C, %e, %M"  ../ExiProcessor/encode_function.sh ${file} ${encodedxml} ${xsdfile}
	...
	/usr/bin/time -o exi_default_decode.txt -a -f "%C, %e, %M"  ../ExiProcessor/decode_function.sh ${encodedxml} ${newxml} ${xsdfile}
	...
	echo "${compressed_size}, ${original_size}, CR:, ${ratio}, ${percentage}% $file">>exi_default_size.txt
